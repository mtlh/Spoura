<script lang="ts">
    ...
    let product: Product = JSON.parse(data.product);
    ...
    let sizes: Array<Number> = [0, 1, 2, 3, 4, 5, 6, 7];
    let count: number = 1;
    function increment () {if (count < 9) {count += 1}};
    function decrement () {if (count > 1) {count -= 1}};

    import { cartProducts, favouriteProducts } from '../../../svelte_stores';
    ...
    const toggleFavourite = async () => {
        if (isFavourite) {
            ...
            await AddFavProduct(product.id)
        }
...
{#if productexists}
    <div class="p-6 m-auto grid grid-cols-1 md:grid-cols-2 max-w-7xl">
        <img src={product.imgurl} alt={product.name} class="w-full h-auto m-auto ring-2 ring-blue-500 rounded-lg">
        <div class="mx-6 my-2">
            ...
            {#if product.col_id}
                <p>Collection: <a class="hover:underline" href="/collection/{product.col_slug}">{product.col_name}</a></p>
            {:else}
                <p class="hidden">Collection: <u>{product.col_name}</u></p>
            {/if}
            ...
            <div class="flex space-x-2 rounded-xl p-2 m-auto">
                ...
                <label tabindex="0" class="btn btn-ghost btn-circle py-2 px-4 my-4" on:click={async ()=> await toggleFavourite()}>
                    <div class="indicator">
                        {#if isFavourite}
