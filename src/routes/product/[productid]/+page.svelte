<script lang="ts">
    ...
    if (Cookies.get("spoura_fav") != undefined) {
        let cookie_arr: any = Cookies.get("spoura_fav")?.split("//");
        for (let num in cookie_arr) {
            if (parseInt(cookie_arr[num]) == product.id) {
                isFavourite = true;
            }
        }
    }
...
{#if product}
    <div class="p-6 m-auto grid grid-cols-1 md:grid-cols-2 max-w-7xl">
        <img src={imgsrc} alt={product.name} class="w-full h-auto m-auto ring-2 ring-blue-500 rounded-lg">
        <div class="mx-6 my-2">
            ...
            <div class="flex space-x-2 rounded-xl p-2 m-auto">
                ...
                <a href="/product/{data.product.id}" data-sveltekit-reload>
